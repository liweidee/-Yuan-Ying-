...
import 'package:yuanying/common/widgets/gesture/horizontal_drag_gesture_recognizer.dart'
    show touchSlopH;
...
class _ViewerState extends State<Viewer> with SingleTickerProviderStateMixin {
    ...
    if (details.pointerCount == 1) {
      if (widget.isLongPic) {
        ...
        if (_scalePos != null && _calc(_scalePos!, details.focalPoint)) {
          ...
          if (drag) {
            _gestureType = _GestureType.drag;
  ...
  void _onScaleUpdate(ScaleUpdateDetails details) {
    if (_gestureType == _GestureType.drag) {
