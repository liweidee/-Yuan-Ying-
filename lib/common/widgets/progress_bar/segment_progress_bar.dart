...
extension ListExtension<T> on List<T> {
  bool isSortedBy(Comparable Function(T) key) {
    for (int i = 1; i < length; i++) {
      if (key(this[i - 1]).compareTo(key(this[i])) > 0) {
        return false;
      }
    }
    return true;
  ...
  // 二分查找最后一个 <= key 的元素
  int lowerBoundByKey(Comparable Function(T) key, double target) {
    int low = 0;
    int high = length;
    while (low < high) {
      final mid = (low + high) ~/ 2;
      if (key(this[mid]).compareTo(target) <= 0) {
        low = mid + 1;
      } else {
        high = mid;
      }
