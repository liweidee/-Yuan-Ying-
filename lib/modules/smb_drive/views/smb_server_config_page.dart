...
class SmbServerConfigPage extends StatelessWidget {
  const SmbServerConfigPage({super.key});
  ...
  Widget build(BuildContext context) {
    ...
    final c = Get.find<SmbServerController>();
  ...
  Widget _emptyState(
    ...
    return Center(
      child: Column(
        ...
        children: [
          Icon(Icons.storage_outlined, size: 64, color: cs.outline),
          const SizedBox(height: 16),
          Text('暂无 SMB 服务器', style: TextStyle(color: cs.outline)),
          const SizedBox(height: 16),
          FilledButton.icon(
            onPressed: onAdd,
            icon: const Icon(Icons.add),
  ...
  Widget _serverCard(
      BuildContext ctx, SmbServerController c, SmbServer s) {
    ...
    return Card(
      ...
      child: ListTile(
        ...
        onTap: () async {
          ...
          Get.toNamed(AppPages.smbMain, arguments: {'serverId': s.id});
        },
      ),
    );
  }

  Future<bool?> _confirmDelete(BuildContext ctx, SmbServer s) {
  ...
  void _openForm(BuildContext ctx, SmbServerController c,
      {SmbServer? existing}) {
...
class _ServerFormDialog extends StatefulWidget {
  ...
  final Future<void> Function(SmbServer server) onSave;
...
class _ServerFormDialogState extends State<_ServerFormDialog> {
  ...
  Future<void> _test() async {
    ...
    final result = await SmbService.instance.testConnection(_collect());
    if (!mounted) return;
    setState(() {
      _testing = false;
  ...
  Widget build(BuildContext context) {
    ...
    return serverDialog(
      title: ServerDialogTitle(
        icon: _isEdit ? Icons.edit_outlined : Icons.storage_outlined,
        title: _isEdit ? '编辑 SMB' : '添加 SMB',
        subtitle: 'SMB / NAS',
      ),
      content: SizedBox(
        width: 440,
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    SwitchListTile(
                      ...
                      title: const Text('匿名访问（Guest）',
                          style: TextStyle(fontSize: 14)),
                      subtitle: const Text('不使用用户名密码登录',
