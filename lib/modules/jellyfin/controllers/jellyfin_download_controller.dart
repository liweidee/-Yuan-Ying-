...
class JellyfinDownloadController extends GetxController {
  ...
  void _loadFromStorage() {
    final stored = StorageManager.getSetting<List<dynamic>>(
          JellyfinStorageKeys.downloadList,
        ) ??
        [];
  ...
  void _saveToStorage() {
    ...
    StorageManager.setSetting(JellyfinStorageKeys.downloadList, list);
  }
  ...
  Future<void> _startDownload(DownloadItem item) async {
    ...
    final downloadDir = Directory('${dir.path}/jellyfin_downloads');
    ...
    try {
      final url =
          JellyfinApiService.getFileStreamUrl(item.baseUrl, item.id, item.token);
      await _dio.download(
        url,
        filePath,
