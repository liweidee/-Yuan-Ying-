...
import '../controllers/jellyfin_search_controller.dart';
import '../services/jellyfin_api_service.dart';

class JellyfinSearchPage extends StatelessWidget {
  const JellyfinSearchPage({super.key});

  @override
  Widget build(BuildContext context) {
    if (!Get.isRegistered<JellyfinSearchController>()) {
      Get.put(JellyfinSearchController());
    }
    final ctrl = Get.find<JellyfinSearchController>();
            ...
            child: Obx(() {
              ...
              return GridView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  final imageUrl = JellyfinApiService.primaryImage(server.baseUrl, item.id, maxWidth: 300);
                  final videoItem = JellyfinConverter.toVideoItem(item, imageUrl);
                  return VideoCardV(
                    videoItem: videoItem,
                    onTap: () => Get.toNamed(AppPages.jellyfinDetail, arguments: {'itemId': item.id}),
                  );
                },
              );
            }),
