...
class JellyfinDownloadPage extends StatelessWidget {
  const JellyfinDownloadPage({super.key});
  ...
  Widget build(BuildContext context) {
    ...
    final ctrl = Get.find<JellyfinDownloadController>();
    final theme = Theme.of(context);
    final serverController = Get.find<JellyfinServerController>();
  ...
  /// 顶部小工具条：任务数 + 清空按钮
  Widget _buildHeader(BuildContext context, JellyfinDownloadController ctrl) {
  ...
  void _playDownloaded(BuildContext context, DownloadItem item) {
    ...
    final videoDetail = VideoDetail(
      vodId: 'jellyfin_download_${item.serverId}_${item.id}',
      vodName: item.name,
      vodPic: '',
      vodContent: '来自 Jellyfin 下载 · 本地播放',
      vodYear: '',
      vodActor: '',
      vodDirector: '',
      vodRemarks: '本地文件',
      typeName: 'Jellyfin 下载',
      playSources: [
  ...
  Future<void> _handleClearAll(
      BuildContext context, JellyfinDownloadController ctrl) async {
    if (ctrl.items.isEmpty) {
  ...
  Widget _buildActionButton(
      DownloadItem item, JellyfinDownloadController ctrl, ThemeData theme) {
    switch (item.status) {
      case DownloadStatus.downloading:
        return IconButton(
