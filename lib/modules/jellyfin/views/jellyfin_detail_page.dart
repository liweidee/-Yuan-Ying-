...
import '../controllers/jellyfin_download_controller.dart';
import '../controllers/jellyfin_favorite_controller.dart';
import '../services/jellyfin_api_service.dart';
import '../services/jellyfin_playback_listener.dart';
...
class JellyfinDetailPage extends StatefulWidget {
  const JellyfinDetailPage({super.key});

  @override
  State<JellyfinDetailPage> createState() => _JellyfinDetailPageState();
}

class _JellyfinDetailPageState extends State<JellyfinDetailPage> {
  ...
  final Rx<JellyfinMediaItem?> item = Rx(null);
  final RxList<JellyfinMediaItem> seasons = <JellyfinMediaItem>[].obs;
  final RxList<JellyfinMediaItem> episodes = <JellyfinMediaItem>[].obs;
  /// 专辑曲目
  final RxList<JellyfinMediaItem> albumTracks = <JellyfinMediaItem>[].obs;
  ...
  final Rxn<JellyfinMediaItem> initialEpisode = Rxn<JellyfinMediaItem>();

  /// 专辑曲目的 URL 缓存（key: trackId, value: streamUrl）
  final Map<String, String> _trackUrlCache = {};

  /// 每个 item 的 mediaSourceId 缓存（key: itemId, value: mediaSourceId）
  ///   播放上报时必须带，否则服务器无法定位媒体源
  final Map<String, String> _trackSourceIdCache = {};

  /// 播放监听的 name→target 映射表（每次播放前清空重建）
  final Map<String, JellyfinPlaybackTarget> _playbackNameToTarget = {};
  ...
  // 计算详情页显示标题
  // ============================================================
  String _effectiveVodName(JellyfinMediaItem item) {
    switch (item.type) {
      case 'Series':
      case 'Movie':
      case 'MusicAlbum':
  ...
  String _buildStreamUrl({
    ...
    if (isAudio) {
      if (supportsDirectPlay) {
        return JellyfinApiService.audioDirectStreamUrl(
          baseUrl, itemId, sourceId, token, container,
        );
      } else {
        return JellyfinApiService.audioUniversalStreamUrl(
          baseUrl, itemId, sourceId, token, container: container,
        );
      }
    }
    if (supportsDirectPlay) {
      return JellyfinApiService.directStreamUrl(
        baseUrl, itemId, sourceId, token, container,
      );
    } else {
      return JellyfinApiService.hlsStreamUrl(
  ...
  Future<String?> _resolveStreamUrl({
    ...
    try {
      ...
      final sourceId = firstSource['Id'] as String? ?? itemId;
      final container =
      ...
      _trackSourceIdCache[itemId] = sourceId; // 缓存 mediaSourceId
      return url;
    } catch (e) {
  ...
  Future<void> _loadDetail() async {
    ...
    try {
      final detail = await api.getItemInfo(
  ...
  Future<void> _loadSeasons(JellyfinServer server, String token) async {
  ...
  Future<void> _loadEpisodes(
      JellyfinServer server, String token, String seasonId) async {
    try {
  ...
  /// 加载专辑曲目
  Future<void> _loadAlbumTracks(JellyfinServer server, String token) async {
    try {
  ...
  Future<void> _play() async {
    ...
    // ===== 电影 / 剧集：走原有逻辑 =====
    _playbackNameToTarget.clear();
    SmartDialog.showLoading(msg: '获取播放信息...');
    try {
      final baseUrl = server.baseUrl;
      final posterUrl =
          JellyfinApiService.primaryImage(baseUrl, detail.id, maxWidth: 400);

      List<PlaySource> playSources = [];
      String defaultStreamUrl = '';
      String? firstEpisodeDisplayName; // 记录第一集的 displayName
          ...
          if (seasonsList.isNotEmpty) {
            ...
            for (final season in seasonsList) {
              ...
              for (final e in episodesList) {
                ...
                episodeEntries.add(Episode(name: displayName, url: url));

                // 收集播放目标
                _playbackNameToTarget[displayName] = JellyfinPlaybackTarget(
                  itemId: e.id,
                  mediaSourceId: _trackSourceIdCache[e.id] ?? e.id,
                );

                // 记录第一集的 displayName
                firstEpisodeDisplayName ??= displayName;
              }
      ...
      // 计算 directTitle（Series 场景用第一集的名字）
      final String directTitle =
          (detail.isSeries && firstEpisodeDisplayName != null)
              ? firstEpisodeDisplayName
              : detail.name;

      // 兜底：如果没有对应的 target，用一个默认的
      _playbackNameToTarget.putIfAbsent(
        directTitle,
        () => JellyfinPlaybackTarget(
          itemId: detail.isSeries ? (itemId) : detail.id,
          mediaSourceId: _trackSourceIdCache[detail.id] ?? detail.id,
        ),
      );

      // 构造播放监听器
      final listener = JellyfinPlaybackListener(
        server: server,
        token: token,
        nameToTarget: Map<String, JellyfinPlaybackTarget>.from(
            _playbackNameToTarget),
      );
  ...
  Future<void> _playAlbum({
    required JellyfinServer server,
    required String token,
    required JellyfinMediaItem album,
    required List<JellyfinMediaItem> tracks,
    required JellyfinMediaItem startTrack,
  }) async {
    _playbackNameToTarget.clear();
    ...
    try {
      final baseUrl = server.baseUrl;
      final posterUrl =
          JellyfinApiService.primaryImage(baseUrl, album.id, maxWidth: 400);
      ...
      for (final track in tracks) {
        ...
        // 收集播放目标
        _playbackNameToTarget[track.name] = JellyfinPlaybackTarget(
          itemId: track.id,
          mediaSourceId: _trackSourceIdCache[track.id] ?? track.id,
        );
      ...
      // 兜底：directTitle 对应的 target
      _playbackNameToTarget.putIfAbsent(
        startTrack.name,
        () => JellyfinPlaybackTarget(
          itemId: startTrack.id,
          mediaSourceId: _trackSourceIdCache[startTrack.id] ?? startTrack.id,
        ),
      );
  ...
  // 播放单集/单曲
  // ============================================================
  Future<void> _playEpisode(JellyfinMediaItem episode) async {
    final server = serverController.currentServer;
    if (server == null || server.userId == null) return;
    final token = serverController.getToken(server.id);
    if (token == null) return;
    ...
    try {
      ...
      // 记录当前集的播放目标
      _playbackNameToTarget[episode.name] = JellyfinPlaybackTarget(
        itemId: episode.id,
        mediaSourceId: _trackSourceIdCache[episode.id] ?? episode.id,
      );
      ...
      if (currentDetail != null) {
        ...
        if (episodes.isNotEmpty) {
          for (final ep in episodes) {
            ...
            allEpisodes.add(Episode(name: displayName, url: epUrl));

            // 收集所有集的播放目标
            _playbackNameToTarget[displayName] = JellyfinPlaybackTarget(
              itemId: ep.id,
              mediaSourceId: _trackSourceIdCache[ep.id] ?? ep.id,
            );
            // 兼容用原始集名作为 key
            _playbackNameToTarget.putIfAbsent(
              ep.name,
        ...
        videoDetail = VideoDetail(
          ...
          vodName: _effectiveVodName(currentDetail),
          vodPic: JellyfinApiService.primaryImage(
  ...
  Future<void> _toggleFavorite() async {
    ...
    try {
      ...
      if (Get.isRegistered<JellyfinFavoriteController>()) {
        Get.find<JellyfinFavoriteController>().refreshData();
      }
    } catch (e) {
  ...
  Future<void> _downloadSingle(
    JellyfinMediaItem target,
    JellyfinServer server,
    String token,
  ) async {
    if (!Get.isRegistered<JellyfinDownloadController>()) {
      Get.put(JellyfinDownloadController());
    }
    final downloadCtrl = Get.find<JellyfinDownloadController>();
  ...
  Future<void> _downloadSeries(
    ...
    List<JellyfinMediaItem> seasonsList = seasons.toList();
    ...
    final selected = await showModalBottomSheet<List<JellyfinMediaItem>>(
      context: context,
  ...
  Future<void> _downloadAlbum(
    JellyfinMediaItem album,
    JellyfinServer server,
    String token,
  ) async {
    List<JellyfinMediaItem> tracks = albumTracks.toList();
    if (tracks.isEmpty) {
    ...
    final List<JellyfinMediaItem> toDownload =
        identical(choice, _albumDownloadAllSentinel)
            ? tracks
            : (choice is JellyfinMediaItem
                ? [choice]
                : <JellyfinMediaItem>[]);
  ...
  Future<void> _downloadEpisodeQuick(JellyfinMediaItem episode) async {
    final server = serverController.currentServer;
    if (server == null) {
      SmartDialog.showToast('未连接服务器');
      return;
                    ...
                    Wrap(
                      ...
                      children: [
                        if (detail.productionYear != null)
                          _buildMetaChip(
                              '${detail.productionYear}', colorScheme),
                        ...
                        if (detail.runtimeTicks != null)
                          _buildMetaChip(
                              _formatDuration(detail.runtimeTicks!),
                              colorScheme),
                        if (detail.isMusicAlbum &&
                            detail.artists.isNotEmpty)
                          _buildMetaChip(
  ...
  Widget _buildTrackItem({
    required JellyfinMediaItem track,
    required String baseUrl,
    required ColorScheme colorScheme,
    required int index,
  }) {
    return GestureDetector(
      onTap: () => _playEpisode(track),
      child: Container(
  ...
  Widget _buildEpisodeItem(
      JellyfinMediaItem episode, String baseUrl, ColorScheme colorScheme) {
    final thumbUrl =
        JellyfinApiService.primaryImage(baseUrl, episode.id, maxWidth: 300);
  ...
  Widget _buildCastSection(
      List<JellyfinPerson> people, String baseUrl, ColorScheme colorScheme) {
                  ...
                  SizedBox(
                    ...
                    child: ClipOval(
                      child: person.hasImage
                          ? Image.network(
                              JellyfinApiService.primaryImage(
                                baseUrl,
                                person.id,
                                maxWidth: 160,
...
class _SeriesDownloadSheet extends StatefulWidget {
  final List<JellyfinMediaItem> seasons;
  final Future<List<JellyfinMediaItem>> Function(String seasonId) loadEpisodes;
...
class _SeriesDownloadSheetState extends State<_SeriesDownloadSheet> {
  final Map<String, List<JellyfinMediaItem>> _episodesBySeason = {};
  ...
  List<JellyfinMediaItem> _collectSelected() {
    final result = <JellyfinMediaItem>[];
    for (final eps in _episodesBySeason.values) {
      for (final e in eps) {
