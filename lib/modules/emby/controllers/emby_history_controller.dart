...
/// Emby 播放历史控制器
///
/// 结构与 [EmbyFavoriteController] / [EmbyLibraryController] 一致：
/// 分页加载 + 下拉刷新 + 无限滚动。
class EmbyHistoryController extends GetxController {
  ...
  /// 从历史中删除单条记录
  ///
  /// 调 Emby 的 `PlayedItems/{itemId}` 取消已播状态，
  /// 成功后本地列表立即移除，无需整页刷新。
  Future<void> removeOne(EmbyMediaItem item) async {
    final server = serverController.currentServer;
    if (server == null || server.userId == null) return;
    final token = serverController.getToken(server.id);
    if (token == null) return;
