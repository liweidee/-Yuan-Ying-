...
class WebDavFileController extends GetxController {
  final serverCtl = Get.find<WebDavServerController>();

  final RxString currentPath = '/'.obs;
  final RxList<WebDavEntry> entries = <WebDavEntry>[].obs;
  ...
  final Map<String, List<WebDavEntry>> _dirCache = {};
  ...
  Future<void> _loadDirectory(
    ...
    try {
      final list =
          await WebDavService.instance.listDirectory(server, path);
      _dirCache[path] = list;
      entries.value = list;
      currentPath.value = path;
    } catch (e) {
  ...
  // 导航
  // ==================================================================
  Future<void> openDirectory(WebDavEntry entry) async {
