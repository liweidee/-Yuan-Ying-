/**
 * HTML解析器工具
 * 移植自道长原版，基于 cheerio 和 JSONPath
 * 依赖: drpy-core-lite.min.js（已包含 cheerio 和 JSONPath）
 */

// 从 drpy-core-lite.min.js 导入依赖
const cheerio = globalThis.cheerio;
const JSONPath = globalThis.JSONPath;

// 内嵌 urljoin（原版从 utils.js 导入，现内嵌）
...
// 创建默认实例
const defaultInstance = new Jsoup();

// 定义独立函数（使用默认实例）
const pdfa = (html, parse) => defaultInstance.pdfa(html, parse);
const pdfh = (html, parse, baseUrl) => defaultInstance.pdfh(html, parse, baseUrl);
const pd = (html, parse, baseUrl) => defaultInstance.pd(html, parse, baseUrl);
const pjfh = (html, parse, addUrl) => defaultInstance.pjfh(html, parse, addUrl);
const pj = (html, parse) => defaultInstance.pj(html, parse);
const pjfa = (html, parse) => defaultInstance.pjfa(html, parse);
const pdfl = (html, parse, list_text, list_url, MY_URL) => defaultInstance.pdfl(html, parse, list_text, list_url, MY_URL);
const pq = (html) => defaultInstance.pq(html);

// 挂载到全局
globalThis.pdfa = pdfa;
globalThis.pdfh = pdfh;
globalThis.pd = pd;
globalThis.pjfh = pjfh;
globalThis.pj = pj;
globalThis.pjfa = pjfa;
...
const lists = {
  append: function(item) {
    if (!globalThis.__LISTS__) globalThis.__LISTS__ = [];
    globalThis.__LISTS__.push(item);
  }
};

const batchFetch = () => { console.warn('batchFetch not implemented'); };
...
const setResult2 = (result) => { globalThis.__RESULT__ = result; };

// 重置 drpy2 状态（清理缓存和全局变量）
function resetDrpyState() {
  // 清空模拟存储（如果存在）
  // 注意：Dart 注入的 local 不在这里，我们无法清空，但可以清空其他全局变量
  const globals = [
    'rule', 'fetch_params', 'rule_fetch_params', 'oheaders',
    'VODS', 'VOD', 'LISTS', 'TABS',
    'MY_URL', 'HOST', 'play_url', 'homeHtmlCache'
  ];
  ...
  if (typeof globalThis.__LISTS__ !== 'undefined') {
    globalThis.__LISTS__ = [];
  }
  if (typeof globalThis.__RESULT__ !== 'undefined') {
...
// ---- 挂载到全局 ----
// req 和 local 由 Dart 注入，我们不覆盖
globalThis.joinUrl = joinUrl;
globalThis.urljoin = joinUrl;      // 别名
globalThis.urlJoin = joinUrl;      // 别名
globalThis.buildUrl = buildUrl;
globalThis.stringify = stringify;
globalThis.lists = lists;
globalThis.batchFetch = batchFetch;
globalThis.print = print;
globalThis.log = log;
globalThis.setResult2 = setResult2;
globalThis.reinit = reinit;
globalThis.resetDrpyState = resetDrpyState;


// ===== 从全局获取依赖 =====
// const cheerio = globalThis.cheerio;
// const 模板 = globalThis.模板;
...
// 注意：JSEncrypt, NODERSA, pako 未在 drpy2.js 中直接使用，可以不获取

// 从全局获取 inject 提供的辅助函数（已由 inject 挂载）
// const joinUrl = globalThis.joinUrl;
// const batchFetch = globalThis.batchFetch;
// 注意：req, local, pd, pdfa, pdfh 都已在 globalThis 中
// pd, pdfa, pdfh 由 htmlParser 挂载，req 和 local 由 Dart 注入

// 确保 pd, pdfa, pdfh 在全局已存在（如果没有，则定义空函数避免报错）
if (typeof globalThis.pd === 'undefined') globalThis.pd = () => '';
if (typeof globalThis.pdfa === 'undefined') globalThis.pdfa = () => [];
if (typeof globalThis.pdfh === 'undefined') globalThis.pdfh = () => '';
